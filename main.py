...
def generate_new_plate(h, w):
    ...
    if manual_plate is not None:
        ...
        return x, y, position, True
    else:
        x = random.randint(0, w - PLATE_SIZE)
        y = random.randint(0, h - PLATE_SIZE)
        position = 'left' if x < w//2 else 'right'
...
while cap.isOpened():
    ...
    if (manual_plate is None) and (current_plate is None) and (cooldown == 0):
        current_plate = generate_new_plate(h, w)
    elif manual_plate is not None:
        current_plate = generate_new_plate(h, w)
    ...
    if results.multi_face_landmarks:
        ...
        cv2.putText(frame, f"Nose: {nose_coord}", (10, h - 100),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, TEXT_COLOR, 2)

        if current_plate:
            x, y, pos, is_manual = current_plate
    ...
    if current_plate:
        x, y, pos, is_manual = current_plate
        color = MANUAL_COLOR if is_manual else PLATE_COLOR
        cv2.rectangle(frame, (x, y), (x + PLATE_SIZE, y + PLATE_SIZE), color, 3)
